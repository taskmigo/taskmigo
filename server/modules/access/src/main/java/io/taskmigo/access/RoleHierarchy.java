...
final class RoleHierarchy {
    ...
    static RoleHierarchy from(Collection<RoleEntity> roles) {
    ...
    List<UUID> reachableFrom(Collection<UUID> roots) {
    ...
    private void requireAcyclic() {
        if (Graphs.hasCycle(this.graph)) throw new AccessException(
