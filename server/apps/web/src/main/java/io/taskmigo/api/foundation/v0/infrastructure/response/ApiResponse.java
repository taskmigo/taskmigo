...
import io.taskmigo.api.foundation.v0.infrastructure.pagination.OffsetPageRequest;
...
public record ApiResponse<T, M extends ApiResponse.Meta>(
    @Schema(description = "Whether the request completed successfully", requiredMode = Schema.RequiredMode.REQUIRED)
    boolean success,
    @Schema(description = "HTTP status code of the response", requiredMode = Schema.RequiredMode.REQUIRED)
    int statusCode,
    @Schema(
        description = "Machine-readable and human-readable outcome message",
    ...
    @Schema(description = "Response payload", requiredMode = Schema.RequiredMode.REQUIRED, nullable = true)
    @Nullable
    T data
) {
    public record Message(
        @Schema(description = "Stable code for the response outcome", requiredMode = Schema.RequiredMode.REQUIRED)
        String code,
        @Schema(
    ...
    public record Error(
        ...
        @Schema(description = "Validation errors keyed by request field name", nullable = true)
        @Nullable
    ...
    public record BasicMeta(
        @Schema(
            description = "Timing information for processing the request",
            requiredMode = Schema.RequiredMode.REQUIRED
        )
        Execution execution
    ) implements Meta {}
    ...
    public record OffsetMeta(
        ...
        Execution execution,
        @Schema(description = "Offset pagination state for this response", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    public record CursorMeta(
        ...
        Execution execution,
        @Schema(description = "Cursor pagination state for this response", requiredMode = Schema.RequiredMode.REQUIRED)
        CursorPagination pagination
    ) implements Meta {}

    public record Execution(
        @Schema(
            description = "Time at which server processing began",
            requiredMode = Schema.RequiredMode.REQUIRED,
            format = "date-time"
        )
        Instant startedAt,
    ...
    public record Offset(
        ...
        int currentPage,
        @Schema(
            description = "Maximum number of items in each page",
            requiredMode = Schema.RequiredMode.REQUIRED,
            minimum = "1"
        )
        int pageSize,
    ...
    public record Cursor(
        @Schema(
            description = "Cursor to retrieve the next page, when one exists",
            requiredMode = Schema.RequiredMode.REQUIRED,
            nullable = true
        )
        @Nullable
        String nextCursor,
        @Schema(
            description = "Cursor to retrieve the previous page, when one exists",
            requiredMode = Schema.RequiredMode.REQUIRED,
            nullable = true
        )
        @Nullable
