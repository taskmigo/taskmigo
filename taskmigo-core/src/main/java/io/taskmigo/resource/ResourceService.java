...
public class ResourceService {
    ...
    ResourceService(OrganizationRepository organizations, UserRepository users, GroupRepository groups, RoleRepository roles,
                    ProjectRepository projects, ProjectMemberRepository projectMembers) {
        this.organizations = organizations; this.users = users; this.groups = groups; this.roles = roles;
        this.projects = projects; this.projectMembers = projectMembers;
    }
    ...
    public UUID createUser(UUID organizationId, String username, String email, String displayName) {
        ...
        try {
            return users.saveAndFlush(new UserEntity(UUID.randomUUID(), organization, required(username, "username"), normalizedEmail,
                required(displayName, "displayName"))).getId();
        } catch (DataIntegrityViolationException exception) {
    ...
    public UUID createRole(UUID organizationId, String name, String description, Set<String> permissions) {
        ...
        return roles.save(new RoleEntity(UUID.randomUUID(), organization(organizationId), required(name, "name"), description, requested)).getId();
    }
    ...
    public UUID addProjectMember(UUID projectId, String principalType, UUID principalId) {
        ...
        try {
            return projectMembers.saveAndFlush(new ProjectMemberEntity(UUID.randomUUID(), project, type, principalId)).getId();
        } catch (DataIntegrityViolationException exception) {
    ...
    public void removeProjectMember(UUID projectId, UUID projectMemberId) {
        ...
        projectMembers.delete(member); projectMembers.flush();
    }
    ...
    public void setProjectMemberRoles(UUID projectId, UUID projectMemberId, Set<UUID> roleIds) {
        ...
        member.roles.clear(); member.roles.addAll(requestedRoles); projectMembers.flush();
    }
    ...
    public Set<String> effectivePermissions(UUID projectId, UUID userId) {
        ...
        projectMembers.findByProjectIdAndPrincipalTypeAndPrincipalId(projectId, PrincipalType.USER, userId)
            .ifPresent(member -> collectPermissions(member, permissions));
        var groupIds = groups.findIdsContainingUser(userId);
        if (!groupIds.isEmpty()) {
            for (ProjectMemberEntity member : projectMembers.findAllByProjectIdAndPrincipalTypeAndPrincipalIdIn(projectId, PrincipalType.GROUP, groupIds)) {
    ...
    private OrganizationEntity organization(UUID id) { return organizations.findById(id).orElseThrow(() -> notFound("Organization not found")); }
    private UserEntity user(UUID id) { return users.findById(id).orElseThrow(() -> notFound("User not found")); }
    private GroupEntity group(UUID id) { return groups.findById(id).orElseThrow(() -> notFound("Group not found")); }
    private ProjectEntity project(UUID id) { return projects.findById(id).orElseThrow(() -> notFound("Project not found")); }
    ...
    private ProjectMemberEntity projectMember(UUID id) { return projectMembers.findById(id).orElseThrow(() -> notFound("Project Member not found")); }
    ...
    private static ResourceException badRequest(String message) { return new ResourceException(ResourceException.Type.BAD_REQUEST, message); }
    private static ResourceException conflict(String message) { return new ResourceException(ResourceException.Type.CONFLICT, message); }
    private static ResourceException conflict(String message, Throwable cause) { return new ResourceException(ResourceException.Type.CONFLICT, message, cause); }
    private static ResourceException notFound(String message) { return new ResourceException(ResourceException.Type.NOT_FOUND, message); }
}
