...
- For every Qodana run, inspect the GitHub Actions job summary for job `Analyze` first. The workflow step `Publish Qodana results to job summary` writes a section headed exactly `## Qodana results` from `qodana.sarif.json`.
- Treat the JSON under `## Qodana results` as the primary source of truth for Qodana source findings. Read its structured result objects for `ruleId`, message, affected file/location, and region data before attempting any repair.
- When Qodana fails, do not infer findings from the generic `Analyze` job conclusion, the `Scan` stdout, a PR comment, or a separate check-run summary. Read `## Qodana results` first. Only fall back to the `qodana-sarif` artifact or GitHub code-scanning annotations if that job-summary section is unavailable, truncated, or lacks enough location detail. Use the `Scan` log only for Qodana startup, project import, Gradle/classpath, cache, or report-generation failures.
