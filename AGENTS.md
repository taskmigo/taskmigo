...
- If the `Formatting` workflow fails, inspect the `Formatting diff` section in the failed `Check formatting` step. The workflow runs Prettier and prints the resulting unified diff directly in the CI log.
