...
| Migration | Managed-resource reconciliation and OAuth client reconciliation execute in one SERIALIZABLE transaction with bounded retry for transient/data-integrity failures. | **CONFIRMED BUSINESS** | DefaultInstallationService; SpringInstallationTransaction; MigrationIntegrationTest |
| Migration | Change events are published only after a successful commit; unchanged resources are not emitted, rolled-back mutations emit no change event, and change events contain resource identity/action rather than credentials. | **CONFIRMED BUSINESS** | DefaultInstallationService; LoggingInstallationChangePublisher; MigrationIntegrationTest |
| Migration | Managed OAuth client secrets use desired-state rotation semantics: an unchanged raw secret preserves the existing encoded value, while a changed raw secret is re-encoded and updates the client. This intentionally differs from initial-only User password semantics. | **CONFIRMED BUSINESS** | InternalClientReconciler; JdbcManagedClientRegistry; SpringPasswordHasher; MigrationIntegrationTest |
| Migration | Executable/module naming and the concrete OAuth persistence adapter/ownership mechanism are not business semantics. | **IMPLEMENTATION DETAIL** | JdbcManagedClientRegistry; migration composition |
