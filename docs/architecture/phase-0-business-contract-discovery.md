...
| Provisioning | The system User is provisioning-owned, requires an initial credential on first creation, and later reconciliation preserves an existing password hash. | **CONFIRMED BUSINESS** | reconcileSystemUser; SystemUserReconciler; migration tests |
...
| M-004 | Installation executable naming is migration rather than bootstrap; OAuth persistence ownership remains an implementation detail. | **IMPLEMENTATION DETAIL** | Preserve reconciliation, system credential, and V1 invariants; allow composition/module names and adapters to change. |
