...
| Group | Group name is required and trimmed; description is optional; child IDs are set-like and must exist. | **CONFIRMED BUSINESS** | DefaultGroupService; GroupApiIntegrationTest |
...
| Role | Role name matches [a-zA-Z0-9_ -]{6,255} and is globally unique. | **CONFIRMED BUSINESS** | AuthorizationName.requiredRole; V1 unique constraint |
...
| Statement | Canonical shape is name, description, effect, scope, target.api.method, target.api.path, and policy. | **CONFIRMED BUSINESS** | REST contract; V1; current spec STMT-001 |
| Statement | Name matches [a-zA-Z0-9_-]{6,255} and is unique; effect/scope/policy are required; policy is non-blank. | **CONFIRMED BUSINESS** | AuthorizationName; StatementPolicyValidator; V1 |
...
| Request authorization | Protected API access requires JWT auth, SCOPE_taskmigo.api, supported principal_type, and valid user_id; there is no system-user API bypass in the manager. | **CONFIRMED BUSINESS** | RequestAuthorizationManager |
...
| Provisioning | Managed Statements/Roles/Users reconcile idempotently by stable business identity and update existing managed state in place. | **CONFIRMED BUSINESS** | provisioning services; BootstrapIntegrationTest |
| Provisioning | The system User is provisioning-owned, requires an initial credential on first creation, and later reconciliation preserves an existing password hash. | **CONFIRMED BUSINESS** | reconcileSystemUser; SystemUserReconciler; bootstrap tests |
| Provisioning | Built-in authorization data is reconciled in dependency order so references are valid: Statements, Roles, then Users/grants. | **CONFIRMED BUSINESS** | AuthorizationStatementReconciler |
...
| Migration | Executable/module naming (bootstrap vs migration) and OAuth persistence ownership are not business semantics. | **IMPLEMENTATION DETAIL** | active PR #99 |
...
| M-004 | Baseline installation executable is bootstrap; active PR #99 moves toward migration and separates OAuth persistence into technical security infrastructure. | **IMPLEMENTATION DETAIL** | Preserve reconciliation, system credential, and V1 invariants; allow composition/module names and adapters to change. |
| M-005 | Provisioning reuses StatementPolicyValidator, so M-001 also affects built-in reconciliation. | **UNRESOLVED** | Do not freeze bootstrap-time semantic validation until M-001 is resolved. |
...
2. Replace **IMPLEMENTATION DETAIL** mechanisms freely, including anemic state surrogates, graph-wide loading, closure rebuild strategy, JPA entity shape, servlet request attributes, and bootstrap executable naming.
