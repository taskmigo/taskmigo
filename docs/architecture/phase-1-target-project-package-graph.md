...
| `:apps:worker`            | **Fixed executable root**                               | Background-job driving adapter and worker composition root. Phase 6 removes unused bounded-context/database dependencies until a real job adapter requires an inbound port.                                                                                                                               |
...
| `:modules:database`       | **Retain shared technical infrastructure**              | Phase 5 confirmed a coherent shared owner: Web and Migration import the datasource configuration, Migration consumes the single V1 Flyway schema, and Identity plus Access Control share generic JPA Criteria comparison mechanics. Architecture enforcement keeps higher-level capability ownership out. |
...
- `:modules:query` remains a standalone supporting capability. Its persistence-neutral predicate representation lives
  ...
  adapters.
- `:modules:database` remains shared technical infrastructure because Web and Migration import its datasource
  configuration, Migration consumes its single V1 Flyway schema, and Identity plus Access Control share its generic JPA
  Criteria comparison helper. The Worker does not depend on Database until a real background job requires persistence.
  Database must not acquire bounded-context or supporting-capability semantics.
