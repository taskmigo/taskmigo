...
- `RolePermission(role_id, permission_key)` is unique.
...
4. Union the permission keys granted by those Roles.
5. Allow the operation when the required permission exists and the resource-level invariant also passes.
